...
export default function Dashboard() {
  const { profile, isAdmin, isTecnico, isFinanceiro, isCliente } = useAuth();
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Total clientes (só admin/financeiro)
      let totalClientes = 0;
  ...
  const formatStatus = (status: string) => {
    const labels: Record<string, string> = {
      ...
      aguardando_terceiros: 'Aguard. Terceiros',
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-navy">{stats.chamadosAbertos}</div>
            {stats.chamadosUrgentes > 0 && (
              <p className="text-xs text-red-500 flex items-center mt-1">
                <AlertTriangle className="h-3 w-3 mr-1" />
                {stats.chamadosUrgentes} urgente(s)
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-petrol">{stats.osEmAndamento}</div>
          </CardContent>
        </Card>
        ...
        {(isAdmin || isFinanceiro) && (
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-navy">{stats.totalClientes}</div>
            </CardContent>
          </Card>
        )}
      </div>
              ...
              {chamadosRecentes.map((chamado) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    <Badge className={getPrioridadeBadge(chamado.prioridade)}>
                      {chamado.prioridade}
                    </Badge>
